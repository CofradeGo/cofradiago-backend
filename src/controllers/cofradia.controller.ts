...
import {
  cofradiaService,
  type ListCofradiasOptions,
  type ActualizarCofradiaInput,
} from "../services/cofradia.service.ts";
...
export const crearCofradia = async (req: Request, res: Response) => {
  try {
    ...
    if (!user)
      return res
        .status(401)
        .json({ message: "Usuario no autenticado. Por favor, inicia sesión." });
    if (user.role !== "DMG")
      return res
        .status(403)
        .json({ message: "Solo usuarios DMG pueden crear cofradías." });
    ...
    return res
      .status(201)
      .json({
        message: `Cofradía "${cofradia.nombre}" creada correctamente`,
        cofradia,
      });
  } catch (error: unknown) {
    if (error instanceof Error) {
      ...
      return res
        .status(500)
        .json({ message: `Error al crear la cofradía: ${error.message}` });
    }
    return res
      .status(500)
      .json({ message: "Error desconocido al crear la cofradía." });
  }
};
...
export const listCofradias = async (req: Request, res: Response) => {
  try {
    ...
    const estadoQuery: "ABIERTA" | "CERRADA" | undefined = req.query.estado as
      | "ABIERTA"
      | "CERRADA"
      | undefined;
    ...
    if (error instanceof Error) {
      ...
      return res
        .status(500)
        .json({ message: `Error al listar cofradías: ${error.message}` });
    }
    return res
      .status(500)
      .json({ message: "Error desconocido al listar cofradías." });
  }
};
...
export const actualizarCofradia = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== "DMG")
      return res
        .status(403)
        .json({ message: "Solo usuarios DMG pueden editar cofradías" });
    ...
    const cofradia = await cofradiaService.actualizarCofradia(
      Number(cofradiaId),
      validData
    );
    return res
      .status(200)
      .json({ message: "Cofradía actualizada correctamente", cofradia });
  } catch (error: unknown) {
    if (error instanceof Error) {
      switch (error.message) {
        ...
        case "COFRADIA_CERRADA":
          return res
            .status(400)
            .json({ message: "No se pueden modificar cofradías cerradas" });
        default:
          console.error("Error al actualizar cofradía:", error);
          return res
            .status(500)
            .json({
              message: `Error al actualizar la cofradía: ${error.message}`,
            });
      }
    }
    return res
      .status(500)
      .json({ message: "Error desconocido al actualizar la cofradía" });
  }
};
...
export const clonarCofradia = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== "DMG")
      return res
        .status(403)
        .json({ message: "Solo usuarios DMG pueden clonar cofradías" });
    ...
    if (!anioNuevo)
      return res
        .status(400)
        .json({
          message: "Debe indicar el año nuevo para la cofradía clonada",
        });
    ...
    if (error instanceof Error) {
      ...
      if (msg.includes("COFRADIA_NOT_FOUND")) {
        return res
          .status(404)
          .json({ message: "Cofradía original no encontrada" });
      }
      ...
      return res
        .status(500)
        .json({ message: `Error al clonar la cofradía: ${msg}` });
    }

    return res
      .status(500)
      .json({ message: "Error desconocido al clonar la cofradía" });
  }
};
...
/**
 * POST /api/v1/cofradias/full
 */
export const crearFullCofradia = async (req: Request, res: Response) => {
  try {
    const user = req.user as User | undefined;
    if (!user)
      return res
    ...
    // Validación mínima: nombre, año y tipo
    if (!payload.nombre || !payload.anio || !payload.tipo) {
      return res
        .status(400)
        .json({
          message: "Datos incompletos: nombre, año y tipo son obligatorios",
        });
    }

    // Llamada al service
    const cofradia = await cofradiaService.createFullCofradia({
      hermandadId: user.hermandadId,
      nombre: payload.nombre,
      anio: payload.anio,
      tipo: payload.tipo,
      cortejos: payload.cortejos || [],
    ...
    return res.status(201).json({
      message: `Cofradía "${cofradia.nombre}" creada correctamente con todos sus elementos`,
      cofradia,
    });
  } catch (error: unknown) {
    if (error instanceof Error) {
      ...
      console.error("Error al crear cofradía completa:", error);
      return res
        .status(500)
        .json({
          message: `Error al crear la cofradía completa: ${error.message}`,
        });
    }
    return res
      .status(500)
      .json({ message: "Error desconocido al crear la cofradía completa." });
  }
};
